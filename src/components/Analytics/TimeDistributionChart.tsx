...
export const TimeDistributionChart: React.FC<TimeDistributionChartProps> = ({ summaries, onOpenFullscreen }) => {
  ...
  const datasets = [
    ...
    {
      label: 'Dead Time / Idle',
      data: summaries.map(s => s.deadTimeHours || 0),
      backgroundColor: CATEGORY_DEFINITIONS.dead_time.color,
      stack: '24h',
    },
    {
      label: '⏳ Unrecorded Gap',
      data: summaries.map(s => s.unnoticedHours || 0),
    ...
    },
    {
      ...
      data: summaries.map(s => s.unloggedHours || 0),
